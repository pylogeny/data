...
def get_multistate_patterns(wordlist, ref="cogid", missing="Ø"):
    patterns = {concept: {doculect: [] for doculect in wordlist.cols} for
            concept in wordlist.rows}
    
    characters = defaultdict(int)
    for idx, doculect, concept, cogid in wordlist.iter_rows("doculect",
            "concept", ref):
        patterns[concept][doculect] += [str(cogid)]
        characters[str(cogid)] += 1
    for concept in patterns:
        for doculect in patterns[concept]:
